    ...
    <q-page class="column q-px-md q-pt-lg">
        <div v-for="role in groups" :key="'role-'+role.dbId">
            ...
            <div class="row q-gutter-x-lg">
                <user-card v-for="user in role.users" :key="'user-'+user.dbId" :user="user"/>
            </div>
        </div>
    ...
    query getTeam($dbId: Int!) {
        team: dbNode(typename:"Team", dbId: $dbId) {
            ... on Team {
                ...
                assignedRoles {
                    role { dbId, name}
                    user { dbId, name, fullName, renderedAvatar}
                }  
            }
        }
    }
...
const groups = computed(() => {
    ...
    const grouped = groupBy(result.value.team.assignedRoles, (r) => {return r?.role?.dbId});
    return Object.values(grouped).map((roles) => {
        return {name: roles[0].role.name, dbId: roles[0].role.dbId, users: roles.map((r) => {return r.user})};
    })
});
