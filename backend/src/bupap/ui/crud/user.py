...
from bupap.avatar import serialize_avatar
from bupap.ui.viewmodel.user import ModifiedUser, NewUser
...
def create_user(user: NewUser, external_session: db.Session | None = None):
    ...
    with db.use_or_open_session(external_session) as session:
        ...
        db_user = db.User(
            name=user.name, full_name=user.full_name, avatar=serialize_avatar(user.avatar)
        )
...
def update_user(user: ModifiedUser, external_session: db.Session | None = None):
    with db.use_or_open_session(external_session) as session:
        db_user = session.get(db.User, user.db_id)
        db_user.name = user.name
        db_user.full_name = user.full_name
        db_user.avatar = serialize_avatar(user.avatar)
